		...
		<div class="main">
			...
			<main class="content">
				<div class="container-fluid p-0">
					...
					<h1 class="h3 mb-3">Halaman <strong>Daftar Lahan Wisata</strong></h1>
								...
								<table id="datatable" class="table table-hover">
									<thead>
										<tr>
											...
											<th style="min-width:100px">Nama Wisata</th>
    ...
    <div class="modal-content">
	  ...
	  <form method="post" action="tambah" id="myForm" enctype="multipart/form-data" accept-charset="utf-8">
		<div class="modal-body">
			...
			<div class="form-group" id="notifikasi_nama_wisata">
				<label for="nama_wisata">Nama Wisata<small style="color:red;vertical-align: top;">*</small></label>
				<input type="text" class="form-control" id="nama_wisata" name="nama_wisata" placeholder="Masukkan Nama Wisata" required="true" autocomplete="off">
			</div>
    ...
    <div class="modal-content">
	  ...
	  <form method="post" action="tambah" id="myForm" enctype="multipart/form-data" accept-charset="utf-8">
		<div class="modal-body">
			...
			<div class="form-group" id="notifikasi_nama_wisata">
				<label for="nama_wisata">Nama Wisata<small style="color:red;vertical-align: top;">*</small></label>
				<input type="text" class="form-control" id="ml-nama_wisata" name="nama_wisata" placeholder="Masukkan Nama Wisata" required="true" autocomplete="off" disabled="true">
			</div>
			<div class="form-group" id="notifikasi_kategori">
				<label for="kategori">Kategori<small style="color:red;vertical-align: top;">*</small></label>
			...
			<div class="form-group" id="notifikasi_desa_id">
				<label for="desa_id">Desa<small style="color:red;vertical-align: top;">*</small></label>
				<select name="desa_id" id="ml-desa_id" class="form-select mb-3" required="true" disabled="true">
					<?php foreach ($desa as $key) { ?>
					<option value="<?=$key['id']?>"><?=$key['nama']?>, <?=$key['kecamatan']?>, <?=$key['kabupaten']?></option>
					<?php } ?>
				</select>
			...
			<div class="row mb-3">
				<div class="col-12">
					<label for="gambar">Foto Wisata<small style="color:red;vertical-align: top;">*</small></label> <br>
...
<!-- <script src="https://unpkg.com/leaflet@1.3.1/dist/leaflet.js"
	...
	$(".tambahModal").click(function (){
		$('#modalnya .modal-header #modallabel').text('Tambah Wisata');
	...
	$(".ubahModal").click(function (){
		let id = $(this).data("id")
		$('#modalnya .modal-header #modallabel').text('Ubah Wisata');
		$('#modalnya #myForm').attr('action','ubah');
		var data = getJSON('<?= site_url('admin/wisata/ambil/') ?>' + id);
		$('#id').val(data.record.id);
		$('#nama_wisata').val(data.record.nama_wisata);
		...
		$('#img-foto-awal').attr("src", "<?= base_url("uploads/wisata") ?>/"+ data.record.foto);
	...
	function showImg(src){
        $('#modal-gambarnya').attr('src', "<?= base_url("uploads/wisata") ?>/" + src)
	...
	$('#myForm, #myForm1, #myForm2, #myForm3').submit(function(e){
        ...
        $.ajax({
            url      : '<?= site_url('admin/wisata/') ?>'+action,
            dataType : 'json',
            type     : 'post',
            data     : dataToSend,
            processData :false,
	...
	$(".lihat_data").click(function (){
		let id = $(this).data("id")
		var data = getJSON('<?= site_url('admin/wisata/ambil/') ?>' + id);
		$('#ml-nama_wisata').val(data.record.nama_wisata);
		...
		$('#ml-img-foto-awal').attr("src", "<?= base_url("uploads/wisata") ?>/"+ data.record.foto);
        ...
        $.confirm({
            ...
            buttons: {
                tryAgain: {
                    ...
                    btnClass: 'btn-green',
                    action: function(){
                        var response = getJSON('<?= site_url('admin/wisata/hapus/') ?>' + id)
                        if(response.status){
